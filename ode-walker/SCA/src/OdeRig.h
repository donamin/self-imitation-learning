...
public:
	...
	vector<int> dof2BoneMapping;
	vector<float> angleSd;
	std::string commandLogFileName = "CommandsLog.txt";

	std::vector<Vector3f> runningAvgLinVel;
	std::vector<Vector3f> lastSavedPos;
	...
	void genericInit(float totalMass)
	{
		...
		runningAvgLinVel.resize(bones.size(), Vector3f::Zero());
		for (size_t i = 0; i < bones.size(); ++i)
		{
			lastSavedPos.push_back(Vector3f(odeBodyGetPosition(bones[i]->body)));
		}
	...
	void saveInitialPose()
	{
		...
		for (size_t i = 0; i < bones.size(); i++)
		{
			...
			b->initialRotation.x() = q[0];
			b->initialRotation.y() = q[1];
			b->initialRotation.z() = q[2];
			b->initialRotation.w() = q[3];
		}
	}
	...
	virtual int computeStateVector(float *out_state, const Quaternionf& stateRotation)
	{
		...
		Vector3f pos_base(odeBodyGetPosition(bones[0]->body));
		out_state[idx++] = pos_base.z();

		Quaternionf q_tmp = ode2eigenq(odeBodyGetQuaternion(bones[0]->body));
		...
		for (size_t i = 0; i < bones.size(); i++)
		{
			...
			pushStateVector3f(idx, out_state, avel);
		}
		...
		for (size_t motorIdx = 0; motorIdx < joints.size(); motorIdx++)
		{
			...
			if (j->nMotorDof == 3)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					out_state[idx++] = odeJointGetAMotorAngle(motor, i);
				}
			}
			else if (j->nMotorDof == 1)
			{
				motor = j->joint;
				out_state[idx++] = odeJointGetHingeAngle(motor);
			}
		}
		return idx;
	}
	...
	void createHinge(int bone1, int bone2, const Vector3f &anchor, const Vector3f &axis, float minAngle, float maxAngle)
	{
		int body1 = bones[bone1]->body, body2 = bones[bone2]->body;
		Joint *j = new Joint();
		j->nMotorDof = 1;
		...
		dof2BoneMapping.push_back(bone2);
		bones[bone2]->joint = j;
		joints.push_back(j);
	}
	...
	void createMotoredBallJoint(int bone1, int bone2, const Vector3f &anchor, const Vector3f &axis0, const Vector3f &axis2, const Vector3f &angleMin, const Vector3f &angleMax)
	{
		int body1 = bones[bone1]->body, body2 = bones[bone2]->body;
		Joint *j = new Joint();
		j->nMotorDof = 3;
	...
	Vector3f getBoneDir(Bone *b) const
	{
		return (getBoneEndPos(b) - getBoneStartPos(b)) / b->length;
	}
	...
	void UpdateRunningAverageLinearVelocity(bool draw = false)
	{
		for (size_t i = 0; i < bones.size(); ++i)
		{
			Vector3f newPos(odeBodyGetPosition(bones[i]->body));
			runningAvgLinVel[i] = 0.3f * runningAvgLinVel[i] + 0.7f * (newPos - lastSavedPos[i]);
			lastSavedPos[i] = newPos;
			if (draw)
			{
				rcSetColor(1, 1, 1, 1);
				Vector3f end = newPos + 5 * runningAvgLinVel[i];
				rcDrawLine(newPos[0], newPos[1], newPos[2], end[0], end[1], end[2]);
			}
		}
	}
};

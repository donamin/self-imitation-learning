...
namespace AaltoGames
{
	...
	float SCAControl::getMachineLearningChosenControlDiscrepancy()
	{
		...
		if (actor_.get()) {
			//When we are here, there are no parallel workers that would require us to use mutexes with actor_.

			actor_->run(old_best_[1].state.data());

			const Eigen::VectorXf& ml_control = actor_->output_operation_->outputs_;
			machine_learning_and_chosen_control_discrepancy_ = (ml_control - old_best_[1].control).norm();
	...
	void SCAControl::getControlL1(int sampleIdx, float * out_control, const float * priorMean, const float * priorStd)
	{
		...
		if (nextSample.particleRole == ParticleRole::OLD_BEST)
		{
			if (next_step_ < (int)old_best_.size()) {
				...
				control = old_best_[next_step_].control;	//nextStep as index because the control is always stored to the next sample ("control that brought me to this state")
		...
		if (nextSample.particleRole == ParticleRole::MACHINE_LEARNING_NO_VARIATION || nextSample.particleRole == ParticleRole::MACHINE_LEARNING_NO_RESAMPLING)
		{
			...
			processed = true;
			//getMachineLearningControlWithContext(currentSample.state.data(),currentSample.previousState.data(),currentSample.previousControl.data(), control.data(), sampleIdx);
	...
	void SCAControl::getControlKL(int sampleIdx, float * out_control, const float * priorMean, const float * priorStd)
	{
		...
		if (!processed)
		{
			...
			if (nextSample.machine_learning_prior_)
			{
				...
				//bool control_available = getMachineLearningControlWithContext(currentSample.state.data(), currentSample.previousState.data(), currentSample.previousControl.data(), control.data(), sampleIdx);
			...
			for (int i = 0; i < control.size(); i++) {
				control[i] = (float)sample_clipped_gaussian(sampling_distributions_[sampleIdx].first[i], sampling_distributions_[sampleIdx].second[i], control_min_(i), control_max_(i)); //Opposed to proposal's sampleWithLimits this doesn't have the granularity of rare areas.
			}

...
void rcDrawCapsule (const float pos[3], const float R[12], float length, float radius, bool writeLog)
{
	RenderCommand c;
	c.type=rcCapsule;
	c.radius=radius;
	...
	if (writeLog)
	{
		char log[MAXCOMMANDLOGLEN];
		sprintf(log, "rcCapsule %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\0", radius, length, pos[0], pos[1], pos[2], c.q.x, c.q.y, c.q.z, c.q.w);
		commandsLog.push_back(std::string(log));
	}
	else
	{
		rcAddCommand(c);
...
void LogFrameInfo(int frameIdx, bool runningPPO, bool finishedPPO, int epoch)
{
	char log[MAXCOMMANDLOGLEN];
	sprintf(log, "Frame %d %d %d %d\0", frameIdx, runningPPO, finishedPPO, epoch);
	commandsLog.push_back(std::string(log));
}

void rcSetupBodyInUnity(int bodyID, const float pos[3], const float R[12], float length, float radius)
{
	RenderCommand c;
	c.type = rcUnityBody;
	c.radius = radius;
	...
	c.b = bodyID;
	char log[MAXCOMMANDLOGLEN];
	sprintf(log, "rcUnityBody %d %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\0", bodyID, radius, length, pos[0], pos[1], pos[2], c.q.x, c.q.y, c.q.z, c.q.w);
	commandsLog.push_back(std::string(log));
	rcAddCommand(c);
}

void rcDrawForceInUnity(int bodyID, const float force[3])
{
	RenderCommand c;
	c.type = rcUnityForce;
	c.b = bodyID;
	c.pos = RCVector3(force);
	char log[MAXCOMMANDLOGLEN];
	sprintf(log, "rcUnityForce %d %.5f %.5f %.5f\0", bodyID, force[0], force[1], force[2]);
	commandsLog.push_back(std::string(log));
	rcAddCommand(c);
}
...
void rcSaveCommandLogs(const char *fileName)
{
	...
	{
		myfile << commandsLog[i].data() << std::endl;
	}
	myfile.close();
}
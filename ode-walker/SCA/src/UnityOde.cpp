...
EXPORT_API void odeJointSetAMotorVelocitiesDegreesPerSecond(int jointId, float vel1, float vel2, float vel3)
{
	...
	{
		dJointID odeJoint = contexts[threadIdx].joints[jointId];
		dJointSetAMotorParam(odeJoint, dParamVel1, vel1*deg2rad);
		dJointSetAMotorParam(odeJoint, dParamVel2, vel2*deg2rad);
		dJointSetAMotorParam(odeJoint, dParamVel3, vel3*deg2rad);
	}
}
...
EXPORT_API void setMotorVelocities(int nJoints, int *jointIds, const float *velocities)
{
	...
	for (int i = 0; i<nJoints; i++)
	{
		...
		case dJointTypeHinge:
		{
			dJointSetHingeParam(odeJoint, dParamVel, velocities[paramIdx++] * deg2rad);
			break;
		}
		case dJointTypeAMotor:
		{
			dJointSetAMotorParam(odeJoint, dParamVel1, velocities[paramIdx++] * deg2rad);
			dJointSetAMotorParam(odeJoint, dParamVel2, velocities[paramIdx++] * deg2rad);
			dJointSetAMotorParam(odeJoint, dParamVel3, velocities[paramIdx++] * deg2rad);
			break;
		}
		default:
